(function(){
  ...
  function subtotal(){return cart.reduce((s,x)=>s+Number(x.product.price||0)*Number(x.qty||0),0)}
  function cardFee(){return checkoutData.payment==='card'?subtotal()*CARD_FEE_RATE:0}
  ...
  function checkoutShell(title,body,action,back){
    const m=$('checkoutModal');if(!m)return;let box=m.querySelector('.checkout-box');if(!box){box=document.createElement('div');box.className='checkout-box';m.appendChild(box)}box.innerHTML=`<div class="sheet-handle"></div><div class="sheet-head"><h2>${title}</h2><button class="close" onclick="hideCheckout()">✕</button></div><div class="ph-progress">${[1,2,3,4].map(n=>`<span class="ph-progress-item ${checkoutStep===n?'active':checkoutStep>n?'done':''}">${checkoutStep>n?'✓':n}</span>${n<4?'<span class="ph-progress-line"></span>':''}`).join('')}</div><div id="checkoutSummary">${body}</div>${back?`<button class="ph-back" onclick="${back}">Назад</button>`:''}${action?`<button class="ph-next" onclick="${action.fn}">${action.text}</button>`:''}`;
  ...
  function renderCheckoutFlow(){
    if(checkoutStep===1){checkoutShell('Контактные данные',`<div class="ph-form-card"><h3>Введите данные</h3><div class="ph-field"><label>Имя *</label><input id="coName" value="${esc(checkoutData.name)}" placeholder="Ваше имя"></div><div class="ph-field"><label>Фамилия *</label><input id="coSurname" value="${esc(checkoutData.surname)}" placeholder="Ваша фамилия"></div><div class="ph-field"><label>Страна *</label><select id="coCountry"><option>Germany</option><option>Poland</option><option>Czechia</option><option>Austria</option><option>Slovakia</option><option>Other</option></select></div><div class="ph-field"><label>Телефон *</label><input id="coPhone" value="${esc(checkoutData.phone)}" placeholder="+49..."></div><div class="ph-field"><label>Email *</label><input id="coEmail" type="email" value="${esc(checkoutData.email)}" placeholder="your@email.com"></div></div>`,{fn:'nextCheckoutStep()',text:'Далее'});setTimeout(()=>{if($('coCountry'))$('coCountry').value=checkoutData.country},0);return;
    }
    if(checkoutStep===2){checkoutShell('Доставка',`<div class="ph-form-card"><h3>Доставка</h3><div class="ph-field"><label>Способ доставки *</label><select id="coDelivery"><option>DPD</option><option>DHL</option><option>GLS</option><option>Самовывоз</option></select></div><div class="ph-fields-2"><div class="ph-field"><label>Город *</label><input id="coCity" value="${esc(checkoutData.city)}" placeholder="Город"></div><div class="ph-field"><label>Почтовый индекс *</label><input id="coPostcode" value="${esc(checkoutData.postcode)}" placeholder="Индекс"></div></div><div class="ph-field"><label>Улица *</label><input id="coStreet" value="${esc(checkoutData.street)}" placeholder="Улица"></div><div class="ph-field"><label>Номер дома *</label><input id="coHouse" value="${esc(checkoutData.house)}" placeholder="Номер дома"></div></div>`,{fn:'nextCheckoutStep()',text:'Далее'},'prevCheckoutStep()');setTimeout(()=>{if($('coDelivery'))$('coDelivery').value=checkoutData.delivery},0);return;
    }
    if(checkoutStep===3){const productsHtml=cart.map(x=>`<div class="ph-review-line"><span>${esc(x.product.name)}${x.flavor?' · '+esc(x.flavor):''} × ${x.qty}</span><b>${money2(x.product.price*x.qty)} EUR</b></div>`).join('');checkoutShell('Проверка заказа',`<div class="ph-review-card"><h3>Ваш заказ</h3>${productsHtml}</div><div class="ph-review-card"><h3>Контакты</h3><div class="ph-review-line"><span>${esc(checkoutData.name)} ${esc(checkoutData.surname)}</span><b>${esc(checkoutData.phone)}</b></div><div class="ph-review-line"><span>${esc(checkoutData.email)}</span><b>${esc(checkoutData.country)}</b></div></div><div class="ph-review-card"><h3>Доставка и оплата</h3><div class="ph-review-line"><span>${esc(checkoutData.delivery)}</span><b>${esc(checkoutData.city)}, ${esc(checkoutData.postcode)}</b></div><div class="ph-review-line"><span>Доставка</span><b>${money2(DELIVERY_FEE)} EUR</b></div><div class="ph-review-line"><span>Оплата</span><select id="coPayment" style="background:#111;color:var(--text);border:1px solid var(--line2);border-radius:9px;padding:6px 8px"><option value="card">Карта</option><option value="cash">При получении</option></select></div><div class="ph-review-line"><span>Комиссия карты 5%</span><b>${money2(cardFee())} EUR</b></div><div class="ph-review-line ph-review-total"><span>К оплате</span><b>${money2(orderTotal())} EUR</b></div></div>`,{fn:'submitCheckoutOrder()',text:'Оформить заказ'},'prevCheckoutStep()');setTimeout(()=>{if($('coPayment')){$('coPayment').value=checkoutData.payment;$('coPayment').onchange=()=>{checkoutData.payment=$('coPayment').value;renderCheckoutFlow()}}},0);return;}
    const order=checkoutData.lastOrder;checkoutShell('',`<div class="ph-success"><div class="ph-success-icon">✓</div><h2>Заказ успешно оформлен</h2><p>Ваш заказ зарегистрирован и передан в обработку.</p><div class="ph-order-code"><div class="ph-order-code-row"><span>Номер заказа</span><b>${esc(order.id)}</b></div><div class="ph-order-code-row"><span>Дата оформления</span><b>${esc(order.date)}</b></div></div></div>`,{fn:'openOrderDetailById('+JSON.stringify(order.id)+')',text:'Информация о заказе'});const wrap=$('checkoutSummary');const extra=document.createElement('button');extra.className='ph-back';extra.textContent='Вернуться на Главную';extra.onclick=()=>{hideCheckout();showCatalog()};wrap.appendChild(extra);
  }
  ...
  window.submitCheckoutOrder=function(){if(!cart.length)return;const orders=loadOrders();const order={id:orderId(),date:orderDate(),status:'Создан',statusKey:'created',customer:{name:checkoutData.name,surname:checkoutData.surname,country:checkoutData.country,phone:checkoutData.phone,email:checkoutData.email},delivery:{method:checkoutData.delivery,city:checkoutData.city,postcode:checkoutData.postcode,street:checkoutData.street,house:checkoutData.house},payment:{method:checkoutData.payment,fee:cardFee()},items:cart.map(x=>({id:x.id,name:x.product.name,flavor:x.flavor||'',qty:x.qty,price:Number(x.product.price||0),img:x.product.img})),subtotal:subtotal(),deliveryFee:DELIVERY_FEE,total:orderTotal()};orders.unshift(order);saveOrders(orders);checkoutData.lastOrder=order;saveCustomer();cart=[];renderCart2();checkoutStep=4;renderCheckoutFlow()};
  ...
  function renderOrders(){const list=$('phOrdersList');if(!list)return;const orders=loadOrders();list.innerHTML=orders.length?orders.map(o=>`<button class="ph-order-card" onclick="openOrderDetailById('${esc(o.id)}')"><div class="ph-order-top"><span class="ph-order-id">${esc(o.id)}</span><span class="ph-order-total">${money2(o.total)} EUR</span></div><div class="ph-order-meta"><span>${esc(o.date)}</span><span>${o.items.length} ${o.items.length===1?'товар':'товара'}</span></div><span class="ph-order-status">${esc(o.status||'Создан')}</span></button>`).join(''):'<div class="ph-no-orders">У вас пока нет заказов.</div>'}
