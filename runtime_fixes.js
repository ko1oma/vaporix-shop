...
function productFor(el){
  const ps=productsList();
  const card=cardFor(el);
  const ids=[el?.getAttribute?.('data-product-id'),el?.getAttribute?.('data-id'),card?.getAttribute?.('data-product-id'),card?.getAttribute?.('data-id')].filter(v=>v!=null&&String(v)!=='');
  ...
  if(m){const key=String(m[1]);const p=ps.find(x=>String(x?.id)===key);if(p)return p;const n=Number(key);if(Number.isInteger(n)&&ps[n])return ps[n]}
  if(card){
    const text=norm(card.textContent);let best=null,len=0;
    for(const p of ps){const n=norm(p?.name);if(n&&text.includes(n)&&n.length>len){best=p;len=n.length}}
    if(best)return best;
  }
  return null;
}
...
function addToCart(p,qty,flavor){
  const c=cartList();
  ...
  window.cart=c;persist(c);
}
async function openFlavorChooser(p){
  if(!p)return;
  await loadFlavors(p);
  ...
  m.querySelector('.vfx-close').onclick=close;
  m.onclick=e=>{if(e.target===m)close()};
  m.querySelectorAll('.vfx-option').forEach(b=>b.onclick=()=>{
    m.querySelectorAll('.vfx-option').forEach(x=>x.classList.remove('active'));
    b.classList.add('active');selected=flavors[Number(b.dataset.i)];qty=1;$('vfxQty').textContent='1';m.querySelector('.vfx-add').disabled=false;
  });
  m.querySelectorAll('[data-q]').forEach(b=>b.onclick=()=>{
    const max=selected?Math.max(1,stock(selected)):999;
    qty=Math.max(1,Math.min(max,qty+(b.dataset.q==='+'?1:-1)));$('vfxQty').textContent=qty;
  });
  m.querySelector('.vfx-add').onclick=()=>{
    if(!selected)return;
    addToCart(p,Math.min(qty,Math.max(1,stock(selected))),selected);close();
    if(typeof window.showCart==='function')window.showCart();
  };
}
...
document.addEventListener('click',function(e){
  ...
  const p=productFor(b);if(!p)return;
  lastClick=now;
  e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
  openFlavorChooser(p);
},true);
...
function armButtons(){
  document.querySelectorAll('button,a,[role="button"]').forEach(el=>{
    if(isAddButton(el)){el.removeAttribute('disabled');el.disabled=false;el.style.pointerEvents='auto';el.setAttribute('aria-disabled','false')}
  });
}
function fixCheckoutScroll(){
  const m=$('checkoutModal');if(!m)return;
  m.style.height='100dvh';m.style.maxHeight='100dvh';m.style.overflow='hidden';
  const b=m.querySelector('.checkout-box');
  if(b){b.style.height='100dvh';b.style.maxHeight='100dvh';b.style.overflowY='auto';b.style.overflowX='hidden';b.style.webkitOverflowScrolling='touch';b.style.touchAction='pan-y';b.style.paddingBottom='130px'}
}
function profileOrders(){
  const p=$('profile');if(!p)return;
  let s=$('phOrdersSection');
  if(!s){s=document.createElement('section');s.id='phOrdersSection';s.innerHTML='<div class="ph-orders-title">Заказы</div><div id="phOrdersList"></div>';const a=p.querySelector('.profile-section-title');p.insertBefore(s,a||p.firstChild)}
  let a=[];try{a=JSON.parse(localStorage.getItem('puffhubOrdersV1')||'[]')||[]}catch(e){}
  const title=s.querySelector('.ph-orders-title');if(title)title.innerHTML='Заказы <span class="ph-orders-count">'+a.length+'</span>';
