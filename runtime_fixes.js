...
/* VAPORIX runtime cleanup V5. Checkout validation is scoped to the checkout form only. */
if(window.__VAPORIX_RUNTIME_CLEAN_V5)return;
window.__VAPORIX_RUNTIME_CLEAN_V5=true;
...
function checkoutFormIsOpen(){
  return checkoutIsOpen() && !!document.querySelector('#checkoutModal .ph-form-card');
}
...
/* The old global validation alert could leak into catalog/profile/cart clicks. */
(function installAlertScope(){
  if(window.__VAPORIX_ALERT_SCOPE_V5)return;
  window.__VAPORIX_ALERT_SCOPE_V5=true;
...
/* Only the checkout entry point checks whether the cart contains a product. */
function installCheckoutGuard(){
  const fn=window.showCheckout;
  if(typeof fn!=='function')return false;
  if(fn.__vaporixEmptyCartGuardV5)return true;
  ...
  guardedShowCheckout.__vaporixEmptyCartGuardV5=true;
  window.showCheckout=guardedShowCheckout;
  window.showCartCheckout=guardedShowCheckout;
  return true;
}

/* Navigation never validates checkout fields. */
function wrapNavigation(name){
  const fn=window[name];
  if(typeof fn!=='function')return false;
  if(fn.__vaporixNavigationCleanV5)return true;
  const wrapped=function(){
    hideNotice();
    return fn.apply(this,arguments);
  };
  wrapped.__vaporixNavigationCleanV5=true;
  window[name]=wrapped;
  return true;
}
...
/* Prevent an outer touch/navigation handler from consuming checkout buttons. */
function installCheckoutButtonInteraction(){
  ...
  ['.ph-cancel-checkout','.ph-back','.ph-next'].forEach(function(sel){
    document.querySelectorAll('#checkoutModal '+sel).forEach(function(btn){
      if(btn.__vaporixTouchV5)return;
      btn.__vaporixTouchV5=true;
      btn.addEventListener('pointerdown',function(e){e.stopPropagation()},{capture:true});
    });
  });
}

/* Same protection for product add/remove controls. Their existing click logic is untouched. */
function installProductButtonInteraction(){
  document.querySelectorAll('.grid .add-cart,.grid .remove,.grid .card-actions button').forEach(function(btn){
    if(btn.__vaporixProductTouchV5)return;
    btn.__vaporixProductTouchV5=true;
    btn.addEventListener('pointerdown',function(e){e.stopPropagation()},{capture:true});
  });
}

/* Keep the checkout itself scrollable; do not lock body scrolling here. */
function installMinimalLayoutFix(){
  let s=el('vaporix-clean-runtime-style-v5');
  if(s)return;
  s=document.createElement('style');
  s.id='vaporix-clean-runtime-style-v5';
  s.textContent=`
#checkoutModal,.checkout-modal{
  position:fixed!important;inset:0!important;width:100vw!important;height:100dvh!important;
  max-width:none!important;max-height:none!important;transform:none!important;box-sizing:border-box!important;
}
#checkoutModal .checkout-box,.checkout-modal .checkout-box{
...
#checkoutModal .ph-cancel-checkout,#checkoutModal .ph-back,#checkoutModal .ph-next,
.checkout-modal .ph-cancel-checkout,.checkout-modal .ph-back,.checkout-modal .ph-next{
  position:relative!important;z-index:30!important;pointer-events:auto!important;touch-action:manipulation!important;
}
.grid .add-cart,.grid .remove,.grid .card-actions button{
...
#vaporixEmptyCartNotice{
  position:fixed!important;inset:0!important;z-index:5000!important;display:flex;align-items:center;justify-content:center;
  padding:20px;background:rgba(0,0,0,.52);opacity:0;pointer-events:none;transition:opacity .12s ease;
}
#vaporixEmptyCartNotice.show{opacity:1;pointer-events:auto}
.vaporix-empty-box{
  width:min(390px,calc(100vw - 40px));padding:24px;border-radius:24px;
...
.vaporix-empty-box button{
  width:100%;height:48px;border:1px solid #4a4850;border-radius:15px;background:#38383e;
  color:#fff;font-weight:900;font-size:16px;touch-action:manipulation;
}
