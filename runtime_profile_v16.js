...
function installProfileObserver(){
  ...
  var obs=new MutationObserver(function(){
    if(window.__VAPORIX_PROFILE_DECORATING_V16)return;
    window.__VAPORIX_PROFILE_DECORATING_V16=true;
    try{enhanceProfile()}finally{window.__VAPORIX_PROFILE_DECORATING_V16=false}
  });
...
function addDetailControls(){
  ...
  if(!head){
    ...
    title.innerHTML='<h2>Информация о заказе</h2><div class="v16-detail-sub">Заказ</div>';
    var top=document.createElement('button');
    top.type='button';top.className='v16-detail-close';top.textContent='✕ Закрыть';
    ...
    head.appendChild(title);head.appendChild(top);
    content.insertBefore(head,content.firstChild);
    var old=content.querySelector('.ph-detail-head');
    if(old)old.remove();
  }else{
    head.className='v16-detail-head';
    var oldClose=head.querySelector('.ph-detail-close');
...
function installRefreshController(){
  ...
  document.addEventListener('click',function(e){
    ...
    try{
      var r=typeof window.refreshSingleOrder==='function'?window.refreshSingleOrder(id):null;
