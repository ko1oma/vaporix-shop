...
/* Keep the existing admin/catalog configuration intact while preventing the
   old VAPORIX branding from winning over the new storefront identity. */
document.write('<script src="https://raw.githubusercontent.com/ko1oma/vaporix-shop/fe04eb945fdbfc3f84e7cfbbda16efd146117e20/config.js"><\\/script>');
...
(function(){
  ...
  function setBrand(){
    ...
    document.querySelectorAll('.logo,.brand').forEach(function(el){
      ...
      b.textContent=' HUB';
      b.style.color='#ff299e';
      el.append(a,b);
    });
  }

  function productList(){
    try{return typeof products==='function'?products():products}catch(e){return []}
  }

  function money(v){return Number(v||0).toFixed(2)}

  function addDirect(index,flavor){
    var list=productList(),p=list && list[index];
    if(!p)return;
    var item={id:p.id,product:p,qty:1};
    if(flavor)item.flavor=flavor;
    var existing=Array.isArray(window.cart)?window.cart.find(function(x){return String(x.id)===String(item.id)&&String(x.flavor||'')===String(item.flavor||'')}):null;
    if(existing)existing.qty+=1;
    else if(Array.isArray(window.cart))window.cart.push(item);
  ...
  function openChooser(index){
    ...
    m.style.cssText='position:fixed;inset:0;z-index:10000;background:rgba(0,0,0,.65);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:20px;';
    var box=document.createElement('div');
    box.style.cssText='width:min(520px,100%);max-height:85vh;overflow:auto;background:#151518;color:#f7f7fa;border:1px solid #4a4a55;border-radius:22px;padding:22px;box-shadow:0 25px 80px rgba(0,0,0,.55);';
    ...
    flavors.forEach(function(flavor){
      ...
      b.type='button';
      b.style.cssText='min-height:52px;border-radius:14px;border:1px solid #4a4a55;background:#1c1c21;color:#fff;text-align:left;padding:10px 12px;font-weight:800;cursor:pointer;';
      ...
      b.onclick=function(){addDirect(index,flavor);closeChooser()};
      grid.appendChild(b);
    });
    box.appendChild(grid);m.appendChild(box);document.body.appendChild(m);
  ...
  /* This is the function used by the catalog buttons in index.html. The
     previous implementation delegated to a missing openAddToCart function,
     so the visible button did nothing. */
  window.addWithQty=function(index){openChooser(Number(index))};

  document.addEventListener('DOMContentLoaded',function(){
    setBrand();
    setTimeout(setBrand,300);
    setTimeout(setBrand,1200);
  });
})();
